...
# Executables
ENKFEXEC=${ENKFEXEC:-${EXECglobal}/enkf.x}
...
SEDATE=$(date -d "${SDATE:0:8} ${SDATE:8:2} ${radassim_delay} hours" +"%Y%m%d%H")
if [[ $CDATE -le $SEDATE && $EXP_WARM_START = ".false." && ! -f $RADSTAT ]]; then
...
&nam_enkf
   ...
   covinflatemax=1.e2,covinflatemin=1,pseudo_rh=${pseudo_rh:-".false."},iassim_order=0,
...
if [ ${write_spread_diag} = ".true." ]; then
   ...
   diagfile[0]=${COMOUT_ATMOS_ANALYSIS_STAT}/${APREFIX}cnvsprd
   diagfile[1]=${COMOUT_ATMOS_ANALYSIS_STAT}/${APREFIX}pcpsprd
   diagfile[2]=${COMOUT_ATMOS_ANALYSIS_STAT}/${APREFIX}oznsprd
   diagfile[3]=${COMOUT_ATMOS_ANALYSIS_STAT}/${APREFIX}radsprd

...
ABIAS=${ABIAS:-${COMOUT_ATMOS_ANALYSIS}/${APREFIX}abias_out.txt}
ABIASPC=${ABIASPC:-${COMOUT_ATMOS_ANALYSIS}/${APREFIX}abias_pc_out.txt}
ABIASAIR=${ABIASAIR:-${COMOUT_ATMOS_ANALYSIS}/${APREFIX}abias_air_out.txt}
ABIASe=${ABIASe:-${COMOUT_ATMOS_ANALYSIS}/${APREFIX}abias_int_out.txt}
fi
...
SADATE=$(date -u -d "${SDATE:0:8} ${SDATE:8:2} + ${assim_freq} hours" +"%Y%m%d%H")

...
from applications_shield import AppConfig
...
class Tasks:
    SERVICE_TASKS = ['arch', 'earc', 'getic', 'eget', 'getfcst', 'archomg']
    VALID_TASKS = ['aerosol_init', 'coupled_ic', 'getic', 'init', 'getfcst',
                   'prep', 'anal', 'sfcanl', 'analcalc', 'analdiag', 
                   'analinc', 'gomg', 'gldas', 'arch',
                   'atmanalprep', 'atmanalrun', 'atmanalpost',
                   'earc', 'ecen', 'echgres', 'ediag', 'efcs',
                   'eobs', 'eomg', 'epos', 'esfc', 'eupd', 'eget',
    ...
    def getic(self):
        ...
        if self.app_config.mode == "forecast-only":
            ...
            deps = []
            for file in files:
                dep_dict = {'type': 'data', 'data': f'&ROTDIR;/{self.app_config.icdump}.@Y@m@d/@H/{file}'}
                deps.append(rocoto.add_dependency(dep_dict))
            ...
            deps.append(rocoto.add_dependency(dep_dict))
            data = f'&ROTDIR;/{self.cdump}.@Y@m@d/@H/atmos/RESTART/'
            ...
            dep_dict = {'type': 'data', 'data': [data,data2], 'age': 30, 'offset': ['-06:00:00','']}
            deps.append(rocoto.add_dependency(dep_dict))
            if self._base.get('EXP_WARM_START', False):
                dep_dict = {'type': 'cycleexist', 'condition': 'not', 'offset': '-06:00:00'}
                deps.append(rocoto.add_dependency(dep_dict))
            dependencies = rocoto.create_dependency(dep_condition='or', dep=deps)
    ...
    def getfcst(self):
        ...
        deps = []
        data = f'&ROTDIR;/{self.cdump}.@Y@m@d/@H/atmos/gdas.t@Hz.atmf006{suffix}'
        dep_dict = {'type':'data', 'data':data, 'offset2': '-06:00:00'}
        deps.append(rocoto.add_dependency(dep_dict))
        data = f'&ROTDIR;/{self.cdump}..@Y@m@d/@H/atmos/gdas.t@Hz.sfcf006{suffix}'
        dep_dict = {'type':'data', 'data':data, 'offset2': '-06:00:00'}
        deps.append(rocoto.add_dependency(dep_dict))
        dependencies = rocoto.create_dependency(dep_condition='nor', dep=deps)

        resources = self.get_resource('getfcst')
        task = create_wf_task('getfcst', resources, cdump=self.cdump, envar=self.envars, dependency=dependencies)
    ...
    def init(self):
        ...
        if self.app_config.mode != "cycled": 
            ...
            deps = []
            for file in files:
                dep_dict = {'type': 'data', 'data': f'&ICSDIR;/{self.app_config.icdump}.@Y@m@d/@H/{file}'}
                deps.append(rocoto.add_dependency(dep_dict))
            dependencies = rocoto.create_dependency(dep_condition='or', dep=deps)
    ...
    def prep(self):
        ...
        if self.app_config.mode == "cycled":
            ...
            deps = []
            dep_dict = {'type': 'task', 'name': f'{self.cdump}getfcst'}
            deps.append(rocoto.add_dependency(dep_dict))
    ...
    def sfcanl(self):
        ...
        if self.app_config.mode == "cycled":        
            ...
            data = f'&ROTDIR;/{self.cdump}.@Y@m@d/@H/atmos/RESTART/'
            data2 = '@Y@m@d.@H0000.coupler.res'
            dep_dict = {'type': 'data', 'data': [data,data2], 'age': 120, 'offset': ['-06:00:00','']}
            deps.append(rocoto.add_dependency(dep_dict))
            dependencies = rocoto.create_dependency(dep_condition='and', dep=deps)
    ...
    def analdiag(self):
        ...
        if self.app_config.mode == "cycled":
            ...
            deps = []
            dep_dict = {'type': 'task', 'name': f'{self.cdump}gomg'}
            deps.append(rocoto.add_dependency(dep_dict))
            dependencies = rocoto.create_dependency(dep=deps)
    ...
    def analinc(self):
        ...
        task = create_wf_task('analinc', resources, cdump=self.cdump, envar=self.envars, dependency=dependencies)
    ...
    def _fcst_forecast_only(self):
        ...
        if self.app_config.do_atm:
            if self._base.get('EXP_WARM_START',False):
                data = f'&ROTDIR;/{self.app_config.icdump}.@Y@m@d/@H/atmos/RESTART/@Y@m@d.@H0000.sfcanl_data.tile6.nc'
                dep_dict = {'type': 'data', 'data': data}
                deps.append(rocoto.add_dependency(dep_dict))
            else:
    ...
    def _fcst_cycled(self):
        ...
        if self.cdump in ['gdas'] or self.app_config.gfsanl:
            dep_dict = {'type': 'task', 'name': f'{self.cdump}sfcanl'}
        else:
            if self.app_config.do_gldas:
                dep_dict = {'type': 'task', 'name': f'gdasgldas'}
            else:
                dep_dict = {'type': 'task', 'name': f'gdasanalcalc'}
    ...
    def eget(self):
        ...
        groups = self._get_hybgroups(self._base['NMEM_ENKF'], self._configs['eget']['NMEM_EARCGRP'], start_index=0)

...
if [ $# -lt 2 ]; then
    ...
    echo ' Syntax: link_shield.sh ( nco | emc | gfdl ) ( cray | dell | hera | orion | jet )'
    exit 1
fi
...
[[ -s gaussian_sfcfcst.exe ]] && rm -f gaussian_sfcfcst.exe
$LINK ../sorc/install/bin/gaussian_sfcfcst.x gaussian_sfcfcst.exe
...
if [[ ! -s ../sorc/gsi.fd/util/zero_biascoeff/build/zero_biascoeff.x ]]; then
  ...
  $LINK ../sorc/gsi.fd/util/zero_biascoeff/build/zero_biascoeff.x .
fi

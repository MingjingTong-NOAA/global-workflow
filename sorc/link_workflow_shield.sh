...
for file in gaussian_c2g_atms.sh gaussian_sfcanl.sh gaussian_sfcfcst.sh gaussian_sfcfcst_nodiagvar.sh ; do
...
[[ -s "shield.x" ]] && rm -f shield.x
${LINK_OR_COPY} "${HOMEgfs}/sorc/shield.fd/SHiELD_build/Build/bin/SHiELD_nh.prod.32bit.intel.x" ./shield.x

...
# Source fix version file
source "${HOMEglobal}/versions/fix_shield.ver"
fi
...
# Link SHiELD fix directories
  ...
  do
    ...
    if [ ${dir} = "gsi" ]; then
      ${LINK_OR_COPY} "${FIX_SHiELD_DIR}/${dir}/${gsi_v2_ver}" "gsi_v2"
    fi
    if [ ${dir} = "aer_shield" ]; then
       ${LINK_OR_COPY} "${FIX_SHiELD_DIR}/${dir}/${aer_shield2022_ver}" "aer_shield2022"
    fi
  done
...
if [[ "${MODEL}" == "shield" ]]; then
  ${LINK_OR_COPY} "${HOMEglobal}/sorc/ufs_utils.fd/ush/run_sfcanl_chgres.sh" .
  for file in cube2gaussian.sh gaussian_c2g_atms.sh gaussian_sfcfcst.sh ; do
    ${LINK_OR_COPY} "${HOMEglobal}/sorc/shield_utils.fd/ush/${file}" .
  done
fi
...
if [[ "${MODEL}" == "shield" ]]; then
  ...
  if [[ -f "${HOMEglobal}/sorc/shield.fd/SHiELD_build/Build/bin/${model_exe}" ]]; then
    ${LINK_OR_COPY} "${HOMEglobal}/sorc/shield.fd/SHiELD_build/Build/bin/${model_exe}" "${model_exe}"
  fi
  for utilexe in fv3_c2g_atms.x gaussian_sfcfcst.x
  do
    [[ -s "${utilexe}" ]] && rm -f "${utilexe}"
    ${LINK_OR_COPY} "${HOMEglobal}/sorc/shield_utils.fd/install/bin/${utilexe}" .
  done
fi
...
if [[ -d "${HOMEglobal}/sorc/gsi_utils.fd/install" ]]; then
    for exe in calc_analysis.x calc_increment_ens_ncio.x calc_increment_ens.x \
        ...
        interp_inc.x recentersigp.x getgribncensmeanp.x zero_biascoeff.x; do
        if [[ -s "${exe}" ]]; then
            rm -f "${exe}"
        fi

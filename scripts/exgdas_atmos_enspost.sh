...
###########################
export POSTGPVARS=${POSTGPVARS:-"KPO=57,PO=1000.,975.,950.,925.,900.,875.,850.,825.,800.,775.,750.,725.,700.,675.,650.,625.,600.,575.,550.,525.,500.,475.,450.,425.,400.,375.,350.,325.,300.,275.,250.,225.,200.,175.,150.,125.,100.,70.,50.,40.,30.,20.,15.,10.,7.,5.,3.,2.,1.,0.7,0.4,0.2,0.1,0.07,0.04,0.02,0.01,"}
...
if [ ${stime} = "anl" ]; then
  ...
  for fhr in $post_times; do
    ...
    $GRIBBIN/grib_copy -w levtype=pl $COMOUT/${PREFIX}pgrb2.1p00.f${fhr3} ./atm.grib
    $GRIBBIN/grib_to_netcdf -D NC_FLOAT -o $COMOUT/${PREFIX}atmf${fhr3}.grib.nc ./atm.grib
    rm ./atm.grib
    $GRIBBIN/grib_copy -w typeOfLevel=heightAboveGround -B'level:i asc' $COMOUT/${PREFIX}pgrb2.1p00.f${fhr3} ./tmp.grib
    $GRIBBIN/grib_copy -w count=1/2/3/7/8/9 ./tmp.grib ./sfc.grib
    $GRIBBIN/grib_to_netcdf -D NC_FLOAT -o $COMOUT/${PREFIX}sfcf${fhr3}.grib.nc ./sfc.grib
    rm ./sfc.grib

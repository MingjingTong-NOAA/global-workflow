...
if [[ "$MODE" == "cycled" ]]; then
   ...
   ABIAS=${ABIAS:-${COMOUT_ATMOS_ANALYSIS}/${APREFIX}abias.txt_out}
   ABIASPC=${ABIASPC:-${COMOUT_ATMOS_ANALYSIS}/${APREFIX}abias_pc.txt_out}
   ABIASAIR=${ABIASAIR:-${COMOUT_ATMOS_ANALYSIS}/${APREFIX}abias_air.txt_out}
   ABIASe=${ABIASe:-${COMOUT_ATMOS_ANALYSIS}/${APREFIX}abias_int.txt_out}
fi
...
if [[ "${JCAP}" -ne "${JCAP_A}" ]]; then
    if [[ ("${DOHYBVAR}" == "YES" || "${ENSREPLAY:-"NO"}" == "YES") && "${JCAP_A}" == "${JCAP_ENKF}" ]]; then
...
EOFunzip
    ...
    "${USHgfs}/run_mpmd.sh" "${DATA}/mp_unzip.sh" && true
    export err=$?
    if [[ ${err} -ne 0 ]]; then
        export pgm="run_mpmd.sh"
        err_exit "run_mpmd failed while unziping diagnotic files."
    fi
...
&OBSQC
  ...
  iasi_cads=${IASI_CADS},blacklst=.false.,
  ${OBSQC}
/
&OBS_INPUT

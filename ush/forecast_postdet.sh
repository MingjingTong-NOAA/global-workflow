...
FV3_out() {
  ...
  if [[ ${NET:-"gfs"} != "shield" ]]; then
    ...
    cpfs "${DATA}/input.nml" "${COMOUT_CONF}/shield.input.nml"
    cpfs "${DATA}/diag_table" "${COMOUT_CONF}/shield.diag_table"
  fi
        ...
        for restart_date in "${restart_dates[@]}"; do
            ...
            for fv3_file in ${file_list}; do
               if [[ ! -s "${DATArestart}/FV3_RESTART/${restart_date}.${fv3_file}" \
                  ...
                  echo "cpfs ${DATArestart}/FV3_RESTART/${fv3_file} ${COMOUT_ATMOS_RESTART}/${restart_date}.${fv3_file}" >> "${cmdfile}"
               else
...
CUBE2GAUS() {
  ...
  C2GSH=${C2GSH:-${USHglobal}/cube2gaussian.sh}
  export GAUATMSEXE=${GAUATMSEXE:-${EXECglobal}/fv3_c2g_atms.x}
  GAUSFCFCSTSH=${GAUSFCFCSTSH:-${USHglobal}/gaussian_sfcfcst.sh}
  export GAUSFCFCSTEXE=${GAUSFCFCSTEXE:-${EXECglobal}/gaussian_sfcfcst.x}

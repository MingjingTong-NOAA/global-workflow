...
FV3_predet() {
    ...
    for ((tt = 1; tt <= ntiles; tt++)); do
        cpreq "${FIXorog}/${CASE}/${orogfix}_oro_data.tile${tt}.nc" "${DATA}/INPUT/oro_data.tile${tt}.nc"
    ...
    cpreq "${FIXglobal}/am/${O3FORC}" "${DATA}/${subdir:-""}global_o3prdlos.f77"
    cpreq "${FIXglobal}/am/${H2OFORC}" "${DATA}/${subdir:-""}global_h2oprdlos.f77"

    # GFS standard input data

    ISOL=${ISOL:-2}

    cpreq "${FIXglobal}/am/global_solarconstant_noaa_an.txt" "${DATA}/${subdir:-""}solarconstant_noaa_an.txt"
    cpreq "${FIXglobal}/am/global_sfc_emissivity_idx.txt" "${DATA}/${subdir:-""}sfc_emissivity_idx.txt"
    ...
    cpreq "${FIXglobal}/am/global_climaeropac_global.txt" "${DATA}/${subdir:-""}aerosol.dat"
    if ((IAER > 0)); then
        local file
        for file in "${FIXglobal}/am/global_volcanic_aerosols"*; do
            cpreq "${file}" "${DATA}/${subdir:-""}$(basename "${file//global_/}")"
        done
    fi
    ...
    cpreq "${FIXglobal}/am/global_co2historicaldata_glob.txt" "${DATA}/${subdir:-""}co2historicaldata_glob.txt"
    cpreq "${FIXglobal}/am/co2monthlycyc.txt" "${DATA}/${subdir:-""}co2monthlycyc.txt"

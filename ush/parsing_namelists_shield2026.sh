...
if [[ "${DOIAU}" = "YES" ]]; then
  cat > diag_table << EOF
  FV3 Forecast
  ${previous_cycle:0:4} ${previous_cycle:4:2} ${previous_cycle:6:2} ${previous_cycle:8:2} 0 0
EOF
  ...
  cat > diag_table << EOF
  FV3 Forecast
  ${current_cycle:0:4} ${current_cycle:4:2} ${current_cycle:6:2} ${current_cycle:8:2} 0 0
EOF
  cat $DIAG_TABLE >> diag_table
...
sCDATE=${model_start_date_current_cycle}
...
&coupler_nml
  ...
  restart_first_time_step = ${write_first_time_step}
  ${coupler_nml:-}
/
...
if [[ "${DOIAU}" == "YES" && "${fcst_wo_da:-"NO"}" == "NO" ]]; then
  cat >> input.nml << EOF
  iaufhrs      = ${IAUFHRS}
...
echo "===================================="
echo "SHiELD_namelists(): 'input.nml'"
cat input.nml
echo "===================================="
}

...
def calcanl_gfs(DoIAU, l4DEnsVar, Write4Danl, ComOut, APrefix, ASuffix,
    ...
    if launcher == 'mpirun':
        ...
        ExecCMDMPI10_host = f'mpirun -np {nvar} --hostfile hosts'
        ...
        ExecCMDMPI10_host = f'mpiexec -l -n {nvar} --cpu-bind depth --depth '+str(NThreads)
    elif launcher == 'srun':
        ...
        ExecCMDMPI10_host = f'srun -n {nvar} --verbose --export=ALL -c 1 --distribution=arbitrary --cpu-bind=cores'
    elif launcher == 'aprun':
        hostfile = os.getenv('LSB_DJOB_HOSTFILE','')
        with open(hostfile) as f:
            hosts_tmp = f.readlines()
        ...
        ExecCMDMPI10_host = 'aprun -l hosts -d '+str(NThreads)+f' -n {nvar}'
    else:
                ...
                if launcher == 'srun': # need to write host per task not per node for slurm
                    ...
                    if os.getenv('SLURM_JOB_PARTITION','') == 'xjet':
                        ...
                        for a in range(0,n1):
                            hostfile.write(hosts[ihost]+'\n')
                        ihost+=1
                        for a in range(0,nvar-1-n1):
                            hostfile.write(hosts[ihost]+'\n')
                    for a in range(0,nvar-1): # need nvar-1 more of the same host for the nvar tasks for chgres_inc
                        hostfile.write(hosts[ihost]+'\n')
            if launcher == 'srun':
...
if __name__ == '__main__':
    ...
    cliptracers = gsi_utils.isTrue(os.getenv('cliptracers', 'NO'))
